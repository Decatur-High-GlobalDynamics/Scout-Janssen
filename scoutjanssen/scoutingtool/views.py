...
def matchPage(request, number):
    ...
    report1 = Report.objects.filter(team_id = matchInfo[0].team1_id).filter(match_id = (Match.objects.filter(number = number).filter(event_id=CurrentScouting.objects.filter(pk = 1).values_list('match_id')[0])))
