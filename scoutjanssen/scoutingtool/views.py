...
def matchPage(request, number):
    ...
    match_id = Match.objects.filter(number = number).filter(event_id=event).values_list('id')[0]
    ...
    report1 = Report.objects.filter(team_id = (matchInfo[0].team1_id)).filter(match_id = (match_id[0]))
