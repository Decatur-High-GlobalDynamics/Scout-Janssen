...
def matchPage(request, number):
    ...
    match_id = Match.objects.filter(number = str(number)).filter(event_id=event)
    ...
    report1 = Report.objects.filter(team_id = str(matchInfo[0].team1_id)).filter(match_id__in = str(match_id))
